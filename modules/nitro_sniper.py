...
class Sniper(Cog):
    ...
    async def on_message(self, message):
        if cds:=c.search(message.content):
            code = cds.group(2)
            if len(code) != 16:
                return
            headers = {
            ...
            r = post(f'https://discord.com/api/v9/entitlements/gift-codes/{code}/redeem', headers={'Authorization': self.bot.http.token, 'content-type': 'application/json'})
            print(r.json())
            await dump_nitro(self.bot, message, r)
