    ...
    async def chatbot(self, ctx: Context, translate: str = None):
        while True:
            ...
            if m.content.lower().strip() == 'exit':
                break
            if translate:
                async with self.bot.http._HTTPClient__session.get('https://api.pgamerx.com/v5/ai', headers={'Authorization': environ['RANDAPI_KEY']}, params={**params, 'message': (await self.translator.translate(m.content, targetlang='en'))['text']}) as r:
                    await m.reply(' > ' + (await self.translator.translate((await r.json())[0]['response'], targetlang=translate))['text'])
            else:
